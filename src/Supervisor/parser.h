...
int show_parsed_data(ParsedXMLElements *pxe)
{
	...
	sprintf(to_send_p,"Problem Name: %s\nUser: %s\nProblem Description: %s\nProblem Purpose: %s", pxe->Description.name.c_str(),pxe->Description.user.c_str(),pxe->Description.problem_id.c_str(),pxe->Description.purpose.c_str());
	send_to_overview(to_send_p);
	
	for(int i = 0; i < (pxe->Tasks.size()); i++)
	{
		cout<< "Checkpoint: " << pxe->Tasks[i].checkpoint << endl;		
	...
	for(set<string>::iterator it = pxe->EMP.checkpoints.begin(); it != pxe->EMP.checkpoints.end(); it++)
	{
		cout << "\t" << *it << endl;
	}
...
int report_validation_error(short error_number)
{
	...
	printf("%s\n",error);
	sprintf(to_send_p,"XML file validation error: %s",error);
	send_to_overview(to_send_p);
	return 1;
}
...
int validate_parsed_data(ParsedXMLElements *pxe)
{
		...
		*/
		...
		for(set<string>::iterator it = pxe->EMP.checkpoints.begin(); it != pxe->EMP.checkpoints.end(); it++)
		{
			if(taskid_index.find(*it) == taskid_index.end())
        ...
        if(pxe->EMP.checkpoints.size() > 0)
		pxe->EMP.emp_enabled = true;
        else
		pxe->EMP.emp_enabled = false;

        for(int i = 0; i < (pxe->Tasks.size()); i++)
	{
		if(pxe->EMP.checkpoints.find(pxe->Tasks[i].task_id) != pxe->EMP.checkpoints.end())
			pxe->Tasks[i].checkpoint = true;	
		else    pxe->Tasks[i].checkpoint = false;
	}

	printf("XML Schema parsed and validated successfully.\n");
	send_to_overview("XML Schema parsed and validated successfully.\n");
	return 0;
...
int report_parse_error(short error_number)
{
	...
	printf("%s\n", error);
	sprintf(to_send_p,"Parsing error: %s",error);
	send_to_overview(to_send_p);
	printf("\nAt character: %d\n", i);
	return 1;
}
...
int parse(XMLFile *xf, ParsedXMLElements *pxe)
{
	...
	/* <checkpoints> */
				...
				if(temp_cp !=  "") pxe->EMP.checkpoints.insert(temp_cp);
