...
void send_to_command_view4(string str)
{	
	set_command_view((char *)str.c_str());		
}

char rec_window1[100];
...
double network_metric(int packet_size, int packet_count, char *supervisor_add )
{
	int temp_garb;
	...
	sprintf(command, "ping -q -s %d -c %d %s > pingstats.tmp", 
			packet_size, packet_count, supervisor_add);
...
void * recalculate_metrics(void *r)
{
        ...
        while(1)
        {
                ...
                nw_metric = network_metric(n->packet_size, n->packet_count, n->supervisor_add);	
                printf("\nMetrics recalculated. Performance Metric: %lf  Network Metric: %lf\n\n", pf_metric, nw_metric);
		sprintf(rec_window1,"Metrics recalculated. \nPerformance Metric: %lf  Network Metric: %lf", pf_metric, nw_metric);
		send_to_command_view4((string)rec_window1);
	}
}
